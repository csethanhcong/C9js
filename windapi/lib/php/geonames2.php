...
if ($connectMySQL = mysql_connect("localhost", "wind_user", "t2i")) {
	if (mysql_select_db("wind", $connectMySQL)) {
		if ($precision) $rq = "SELECT * FROM geonames WHERE name = '" . $place . "' ORDER BY population DESC";
		else $rq = "SELECT * FROM geonames WHERE name LIKE '" . $place . "%' OR ansiname LIKE '" . $place . "%' OR alternatenames LIKE '%" . $place . "%' ORDER BY population DESC";
		$res = mysql_query($rq);
